...
class MotionAnimation extends JPanel implements ChangeListener, ActionListener 
{
	...
	public MotionAnimation() throws IOException 
	{
		...
		JLabel titleLabel = new JLabel("<html> <i> <font color = blue> Saturation Flow </font></i></html>         ",JLabel.CENTER);
		...
		SaturationFlowExpApplet.cbStarttime = new JComboBox(strttime);
		SaturationFlowExpApplet.cbDuration = new JComboBox(duration);
		SaturationFlowExpApplet.cbCountinterval = new JComboBox(countinterval);
		lHours = new JLabel((String) SaturationFlowExpApplet.cbStarttime.getSelectedItem());
		if(((String) SaturationFlowExpApplet.cbStarttime.getSelectedItem()).length() == 1)
			lHours.setText("0"+(String) SaturationFlowExpApplet.cbStarttime.getSelectedItem());
		else
			lHours.setText((String) SaturationFlowExpApplet.cbStarttime.getSelectedItem());
		...
		pane2.add(SaturationFlowExpApplet.cbStarttime);
		pane2.add(lDuration);
		pane2.add(SaturationFlowExpApplet.cbDuration);
		pane2.add(lCountinterval);
		pane2.add(SaturationFlowExpApplet.cbCountinterval);
		pane2.add(lHours);
		pane2.add(lColon1);
		pane2.add(lMinutes);
		pane2.add(lColon2);
		...
		ActionListener taskPerformerVehicle = new ActionListener()
        {
			...
			public void actionPerformed(ActionEvent evt)
        	{
        		if(timerForVehicle.isRunning() )
        		{
        			...
        			else
        			{
        				...
        				for (int i = 0; i < SaturationFlowExpApplet.xCord.length; i++)
        					SaturationFlowExpApplet.xCord[i] = 0;        
        				SaturationFlowExpApplet.bNext.setEnabled(true);
			...
			private void updateVehiclePosition() 
			{
				...
				for (int i = 0; i < SaturationFlowExpApplet.maxVeh; i++)
				{
					if(timeCheck < 1 && time != INITIAL_TIME+1)
					{
						if((Double.parseDouble(SaturationFlowExpApplet.map[time+1][i+1]) > 0) && (Double.parseDouble(SaturationFlowExpApplet.map[time][i+1]) > 0))
							SaturationFlowExpApplet.xCord[i] += ((Double.parseDouble(SaturationFlowExpApplet.map[time+1][i+1])-Double.parseDouble(SaturationFlowExpApplet.map[time][i+1]))/((double)1/addTimeCheck) * (xSize/SaturationFlowExpApplet.maxLength));//....interpolate the value
							//SaturationFlowExpApplet.xCord[i] += i+4 ;//....interpolate the value
					...
					{
						SaturationFlowExpApplet.xCord[i] = Double.parseDouble(SaturationFlowExpApplet.map[time][i+1]) * (xSize/SaturationFlowExpApplet.maxLength) - IMAGE_WIDTH;
						 // SaturationFlowExpApplet.xCord[i] = i;
			...
			/*	for (int i = 0; i < SaturationFlowExpApplet.maxVeh/3; i++)
				{
					if(timeCheck < 1 && time != INITIAL_TIME+1)
					{
						if((Double.parseDouble(SaturationFlowExpApplet.map[time+1][i+1]) > 0) && (Double.parseDouble(SaturationFlowExpApplet.map[time][i+1]) > 0))
							SaturationFlowExpApplet.CRyCord[i] += ((Double.parseDouble(SaturationFlowExpApplet.map[time+1][i+1])-Double.parseDouble(SaturationFlowExpApplet.map[time][i+1]))/((double)1/addTimeCheck) * (ySize/50));//....interpolate the value
							//SaturationFlowExpApplet.xCord[i] += i+4 ;//....interpolate the value
					...
					{
						SaturationFlowExpApplet.CRyCord[i] = (int) Double.parseDouble(SaturationFlowExpApplet.map[time][i+1]) * (ySize/50) - IMAGE_HEIGHT;
						 // SaturationFlowExpApplet.xCord[i] = i;
				...
				for (int i = 0; i < SaturationFlowExpApplet.xCord.length; i++)
				{
					if(time == INITIAL_TIME+1)
						SaturationFlowExpApplet.yCord[i] = ySize/4+50 + IMAGE_HEIGHT/4 ;
					
					
					
					/*if(i < SaturationFlowExpApplet.maxVeh-1 && (int)SaturationFlowExpApplet.xCord[i] - (int)SaturationFlowExpApplet.xCord[i+1]<= (IMAGE_WIDTH + GAP) && SaturationFlowExpApplet.xCord[i] > 0 && SaturationFlowExpApplet.xCord[i+1] > 0 && SaturationFlowExpApplet.yCord[i] == SaturationFlowExpApplet.yCord[i+1])
					{
						SaturationFlowExpApplet.yCord[i+1] = SaturationFlowExpApplet.yCord[i+1] + IMAGE_HEIGHT + GAP;
					...
					*/
					...
					/*if(i < SaturationFlowExpApplet.maxVeh-2 && (int)SaturationFlowExpApplet.xCord[i] - (int)SaturationFlowExpApplet.xCord[i+2]<= (IMAGE_WIDTH + GAP) && SaturationFlowExpApplet.xCord[i] > 0 && SaturationFlowExpApplet.xCord[i+2] > 0 && SaturationFlowExpApplet.yCord[i] == SaturationFlowExpApplet.yCord[i+2])
					{
						SaturationFlowExpApplet.yCord[i+2] = SaturationFlowExpApplet.yCord[i+2] + IMAGE_HEIGHT + GAP;
					}*/
...
*/
	...
	protected void calculateTotal() 
	{
		...
		int rowtotal = 0;
		for(int i = 0; i < SaturationFlowExpApplet.vRow.size();i++)
		{
			rowtotal = 0;
			Vector<Integer> vData = SaturationFlowExpApplet.vRow.get(i);
	...
	public void paintComponent(Graphics g)
	{
	    ...
	    setDetPosition1((int) (((float)(SaturationFlowExpApplet.maxLength - AREALENGTH)/2) * (xSize/SaturationFlowExpApplet.maxLength)));
        setDetPosition2((int) (xSize - (((float)(SaturationFlowExpApplet.maxLength - AREALENGTH)/2) * (xSize/SaturationFlowExpApplet.maxLength))));
	    
        //System.out.println(getDetPosition2()-getDetPosition1());
        //System.out.println(AREALENGTH*(xSize/SaturationFlowExpApplet.maxLength));
	...
	public void paint(Graphics g) 
    {
    	...
    	for(int i = 0;i < SaturationFlowExpApplet.maxVeh; i++)
    	{
    		if((int)SaturationFlowExpApplet.xCord[i] > 0){
    			g2d.drawImage(SaturationFlowExpApplet.image[i], (int)SaturationFlowExpApplet.xCord[i], SaturationFlowExpApplet.yCord[i], this);//draw the vehicles at the positions calculated..
    	...
    	/*for(int i = 0;i < SaturationFlowExpApplet.maxVeh/3; i++)
    	{
    		if((int)SaturationFlowExpApplet.CRyCord[i] < ySize/7+240)
    			g2d.drawImage(SaturationFlowExpApplet.CRimage[i], (int)SaturationFlowExpApplet.CRxCord[i], SaturationFlowExpApplet.CRyCord[i], this);	
	...
	{
		maxTime = Integer.parseInt((String) SaturationFlowExpApplet.cbDuration.getSelectedItem()) * 60;
		...
		SaturationFlowExpApplet.cbStarttime.setEnabled(false);
		SaturationFlowExpApplet.cbDuration.setEnabled(false);
		SaturationFlowExpApplet.cbCountinterval.setEnabled(false);
	}
