...
import main.SaturationFlowExpApplet.CarMotion;

public  class SaturationFlowExpApplet extends JApplet implements ActionListener,ChangeListener 
{
...
//	int CRimgWidth,CRimgHeight; //Cross Road image height and width
	...
	private double AcceptedGapTime,RejectedGapTime;
	int yTopLane;
	...
	{  
		...
		try 
		{
			...
			*/
			...
			//CRxCord = new double[maxVeh/3];
			//CRyCord = new int[maxVeh/3];
			...
			for(int j = 0; j < maxVeh; j++)
				{
					if(vtype[j].equals("CR"))
					{
						
		        		Image temp1 = CRimage[j];
		        		CRimage[j] = temp1.getScaledInstance(IMAGE_HEIGHT, IMAGE_WIDTH, Image.SCALE_FAST);
		        		if (CRimage[j] == null)
		        			System.out.println("Image not loaded");
		...
		//	carmotion1 = new CarMotion(CRimage);
	        ...
	        ActionListener taskPerformerTime = new ActionListener()
	        {
	        	...
	        	public void actionPerformed(ActionEvent evt)
	        	{
					...
					AcceptedGapTime=(timeOfTimer);
					}
	        		
	        		else
					{
					lTimer.setText(timeOfTimer+" seconds");
					RejectedGapTime=(timeOfTimer);
					}
			...
			timerForCRVehicle = new Timer(300 , taskPerformerCRVehicle);//fires event after 100 millisec. for constant refresh rate for updation of vehicle position
    ...
    {	
    	...
    	public void paint(Graphics g) 
        {
        	...
        	startlinepos=stoplineposition+200;
	...
	public void readData(URLConnection urlConn) throws IOException
	{
		...
		CRmaxVeh = Integer.parseInt(map[1][1])/2;
	...
	{ 
		System.out.println(" update vehicle CR called");
		...
		for (int i = 0; i < maxVeh; i++)
		{
			...
			{
				if((Double.parseDouble(map[time+1][i+1]) > 0) && (Double.parseDouble(map[time][i+1]) > 0))
					CRyCord[i] += ((Double.parseDouble(map[time+1][i+1])-Double.parseDouble(map[time][i+1]))/((double)1/addTimeCheck) * (xSize/maxLength));//....interpolate the value
					CRxCord[i] += xSize/7+50+650;
			}
			else
			{
				
				CRyCord[i] = Double.parseDouble(map[time][i+1]) * (xSize/maxLength) - IMAGE_WIDTH;
				CRxCord[i] += xSize/7+50+650;
				//System.out.println(" TIME :"+time +" , xCord: "+i+": "+xCord[i]);
			}
